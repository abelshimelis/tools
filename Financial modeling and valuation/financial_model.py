...
class FinancialModel:
    def __init__(self, statements, assumptions, owc, depreciation, debt):
        ...
        self.assumptions.index = self.assumptions.index.astype(str).map(self._normalize_label)
    ...
    def a(self, label, year):
        """Get assumption value from Excel. Raises error if missing."""
        ...
        try:
            ...
            if label in self.assumptions.index and year in self.assumptions.columns:
                row = self.assumptions.loc[label]
                years = list(self.assumptions.columns)
                idx = years.index(year)
                for prev in range(idx - 1, -1, -1):
                    candidate = row.iloc[prev]
                    if not pd.isna(candidate):
                        return float(candidate)
                for nxt in range(idx + 1, len(years)):
                    candidate = row.iloc[nxt]
                    if not pd.isna(candidate):
                        return float(candidate)
            raise ValueError(
                f"\n  MISSING ASSUMPTION: '{label}' for {year}\n"
                f"  → Please fill this in your Assumptions sheet."
            )

    def _hist(self, label, stmt='income'):
        """Get the 2024A (last historical) value from income or balance sheet.
        Raises error if missing — do not use 0.0 silently."""
        ...
        try:
            ...
            sheet = 'Income Statement' if stmt == 'income' else 'Balance Sheet'
            raise ValueError(
                f"\n  MISSING HISTORICAL DATA: '{label}' for 2024A in '{sheet}' sheet\n"
                f"  → Please fill this in your Excel template."
            )
